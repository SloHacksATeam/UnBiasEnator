...
def lowest_mag(list, client, original):
	min_i = -1
	...
	if min_i == -1:
		return original
	if min_mag > SENTIMENT_THRESHOLD:
		return None
	
	return list[min_i]

def normalize_query(list, subjects, client, dictionary):
	new_query = []
	for i in range(len(list)):
		word = list[i]
		if word not in subjects:
			new_word = lowest_mag(dictionary.synonym(word), client, word)
			if new_word != None:
...
def main():
	...
	new_query = normalize_query(tokens, subjects, client, dictionary)
